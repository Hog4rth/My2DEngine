...
// --- Tag Functions --- **In the future the function will be removed and all the info will be stored in different .ini
// files (Player.ini, Solid.ini...)**

void GameEngine::CreatePlayer(const int entityID, const float positionX, const float positionY, const float width,
                              const float height) {
...
void GameEngine::CreateSolid(const int entityID, const float positionX, const float positionY, const float width,
                             const float height) {
	if (entityID < 0 || entityID >= MAX_ECS_ENTITIES) {
		std::cerr << "Error: Entity ID " << entityID << " is out of bounds!\n";
		return;
