...
void CollisionSystem::UpdateCollisions(TagComponent tags[], PositionComponent positions[], SizeComponent sizes[], CollisionComponent colliders[], VelocityComponent velocities[], float deltaTime, const int MAX_ECS_ENTITIES) {
	...
	for (int i = 0; i < MAX_ECS_ENTITIES; ++i) {
		...
		if (velocities[i].vx == 0 && velocities[i].vy == 0) { // check if i-object is moving
			continue;
		}
		...
		for (int j = 0; j < MAX_ECS_ENTITIES; ++j) {
			...
			float futurePositionX = positions[i].x + velocities[i].vx * deltaTime;
			...
			if (futurePositionX + sizes[i].width > positions[j].x &&
				...
				if (velocities[i].vx > 0) { // and if i-object is moving right
					velocities[i].vx = (positions[j].x - positions[i].x - sizes[i].width) / deltaTime; // adjust the velocity to stop at the left edge of j-object
					colliders[i].onTheRightWall = true;
				}
				else if (velocities[i].vx < 0) { // and if i-object is moving left
					velocities[i].vx = (positions[j].x + sizes[j].width - positions[i].x) / deltaTime; // adjust the velocity to stop at the right edge of j-object
					colliders[i].onTheLeftWall = true;
				}
			}

			futurePositionX = positions[i].x + velocities[i].vx * deltaTime; // Calculate again because velocity may have changed
			float futurePositionY = positions[i].y + velocities[i].vy * deltaTime;
			...
			if (futurePositionX + sizes[i].width > positions[j].x &&
				...
				if (velocities[i].vy > 0) { // and if i-object is moving down
					velocities[i].vy = (positions[j].y - positions[i].y - sizes[i].height) / deltaTime; // adjust the velocity to stop at the top edge of j-object
					colliders[i].isOnTheGround = true;
				}
				else if (velocities[i].vy < 0) { // and if i-object is moving up
					velocities[i].vy = (positions[j].y + sizes[j].height - positions[i].y) / deltaTime; // adjust the velocity to stop at the bottom edge of j-object
				}
			}
		}
	}
